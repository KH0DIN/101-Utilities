...
local local_player = gameGetService(Players).LocalPlayer

local lines = hash_listsplit(n)
local clock_offset
...
if not found_hash then
   ...
   appendfile(alt_hashes.txt, tostring(local_player.UserId)....tostring(clock_offset)..n)
end
